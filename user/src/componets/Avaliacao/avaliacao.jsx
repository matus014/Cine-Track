...
export default function Avaliacao(props) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (i < numEstrela) {
            ...
            estrelas.push(<FaStar key={i} color="gold" fontSize={20} />);// Adiciona em estrelas o icon da estrela pintada

        } else if (i === numEstrela && meiaEstrela) {

            estrelas.push(<FaStarHalfAlt key={i} color="gold" fontSize={20} />);// Adiciona em estrelas o icon da estrela pintada pela metade

        } else {

            estrelas.push(<FaRegStar key={i} color="gold" fontSize={20} />);// Adiciona em estrelas o icon do contorno da estrela
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center' }}>
            {estrelas}
            <span style={{ marginLeft: '4px', marginTop:"0px", fontSize: "16px", color:"white", }}>
                {notaEmCinco} / 5
